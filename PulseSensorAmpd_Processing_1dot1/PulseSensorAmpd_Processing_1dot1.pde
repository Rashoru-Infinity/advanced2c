...
Serial port; 
PrintWriter pnnTelegraf; //for writing pNN50 in text file
int IBI_prev=0;
int BPM_prev=0;
int checker=0;
double pNN=0.0;
int pNN_C=0;
int pNN_n=0;
double pNN_checker=0.0;
int[] pNN_data = new int[30];
double pNN_aver=0.0;
int datalist=0;
int pNN_num=0;
double[] pNN_index = new double[40000];
...
void draw() {
    ...
    int ho=hour(); 
    if(BPM!=0&&IBI!=0){
        if(BPM==BPM_prev&&IBI==IBI_prev){
            checker=1;
        }
        if(checker!=1){
            println(ho+":"+m+":"+s);
            //pNN50
            if(IBI_prev!=0&&BPM_prev!=0){
                pNN_n=IBI-IBI_prev;
                    if(pNN_n*pNN_n>=2500){
                        pNN_data[pNN_C]=1;
                    }else{
                ...
                if(datalist == 1){
                    ...
                    pNN_aver=pNN_aver/30;
                    pnnTelegraf=createWriter("../local/pnn");
                    pnnTelegraf.print(pNN_aver * 100);
                    pnnTelegraf.close();
                    if(pNN_aver>=0.3) {
                        println("confortable");
                    }
                    if(pNN_aver<0.3) {
                        println("unconfortable");
                    }
                    println(pNN_aver);
                    pNN_index[pNN_num]=pNN_aver;
                    println("pNNnumber"+pNN_num);
                    pNN_num++; //acquired pNN50 num
                }
            }
            IBI_prev=IBI;
            BPM_prev=BPM;
